...
function getSlopeLimits(side) {
  if (side === "BUY") {
    return { slopeMin: SLOPE_CFG.up_weak.min, slopeMax: SLOPE_CFG.up_extreme.min };
  } else {
    return {
      slopeMin: Math.abs(SLOPE_CFG.down_weak.max),
      slopeMax: Math.abs(SLOPE_CFG.down_extreme.max) || Math.abs(SLOPE_CFG.up_extreme.min),
    };
  }
}
