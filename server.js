...
const VOL_CFG = {
  ...
  EURGBP:      { lowMax: 0.000277, medMax: 0.000519, highMax: 0.000828 },
...
function volRegime(symbol, atr, price) {
  if (!Number.isFinite(atr) || !Number.isFinite(price) || price <= 0) return "unknown";
  const cfg = VOL_CFG[symbol] ?? VOL_CFG.default;
...
app.get("/api/debug/volatility", (req, res) => {
  const scan = CACHE.scan ?? [];

  const rows = scan
    .filter(r => r.symbol)
    .map(r => {
      const atr   = num(r.atr_m15);
      const price = num(r.price) || num(r.close);
      const ratio = atr && price ? atr / price : null;

      return {
        symbol:    r.symbol,
        atr_m15:   atr,
        price,
        ratio:     ratio ? Number(ratio.toFixed(8)) : null,
        regime:    volRegime(r.symbol, atr, price),
        thresholds: VOL_CFG[r.symbol] ?? VOL_CFG.default,
      };
    })
